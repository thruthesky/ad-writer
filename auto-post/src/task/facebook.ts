...
class Facebook extends Nightmare{
    ...
    private async login(){
        ...
        if ( await re ){  this.success("You are NOT in login page"); protocol.send('login', 'success') }
        else{ this.failure("You are STILL in login page");protocol.error('login','failed');
        }
